...
11. ✅ Third playtest fixes: (a) closed a resubmission race where the guess form's submit button re-enabled before the round-trip listener had flipped the screen to "locked," so a second tap could send a second write the (still-unpublished at the time) rules rejected — now the screen locks the instant a submit succeeds, no round-trip wait; (b) fixed a real actor-matching bug (see "Flexible actor-name matching" below) where a 1-2 letter guess could score a point via the substring check; (c) host screen redesigned to use most of the viewport width (`.cover-inner` max-width 640px → `min(92vw, 1400px)`) and the per-round team results / scoreboard switched from a single vertical stack to a responsive grid, so a multi-team round uses horizontal space instead of growing tall enough to need scrolling.
12. PC-TV kiosk polish, then the phone-mirroring fallback. Still untested: a real multi-team playtest with a full 30–50 clip library.
...
`namesAreClose(a, b)` in both host/app.js and player/app.js (kept in sync manually, same duplication pattern as the rest of the scoring logic): normalizes (strip accents/punctuation, lowercase), then — provided *both* names are at least 3 characters after normalizing — matches if exactly equal, if one name contains the other as a substring ("DiCaprio" → "Leonardo DiCaprio"), if the last words match ("Deniro" → "De Niro"), or via Levenshtein distance scaled to the name's length (~1 allowed slip per 4 characters, minimum 1) rather than a flat cutoff. An actor guess counts if it's close to *any* name in the clip's cast. The 3-character floor (2026-08-03 fix) exists because the substring check alone made a 1-2 letter guess like "A" or "W" trivially "contain"/"be contained in" almost any longer cast name — a real bug caught in a live playtest (a guess of "A" scored a point). Below that length only an exact match counts; real first/last names are essentially never shorter than that, so this doesn't cost any legitimate matches.
