...
7. ✅ Bonus trivia: admin-tagging can optionally attach multiple-choice trivia questions to a clip; after some reveals (~50% chance, only for clips that have trivia) the host's Next-clip button leads into a Kahoot-style round instead — question + 4 colored options on the TV, every team (not just the round's active team) races to answer on their phone, first correct team wins 5 points.
8. PC-TV kiosk polish, then the phone-mirroring fallback. Still untested: a real multi-team playtest with a full 30–50 clip library.
