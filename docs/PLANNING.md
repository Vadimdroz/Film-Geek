...
5. ✅ Audio-only round phase: each round plays audio-only for 15s first (video hidden, cover stays up), during which each team can independently tap "Answer now" (skip straight to guessing, double points) or "Go to video" (clip continues into video from where it already is, visible, normal scoring); auto-continues to video if a team doesn't choose in time.
...
10. PC-TV kiosk polish, then the phone-mirroring fallback. Still untested: a real multi-team playtest with a full 30–50 clip library.
...
```
rooms/{roomCode}                        — public: hostUid, phase (lobby|audio|playing|guessing|revealed|trivia|trivia-revealed), roundIndex, guessDeadline, revealedAnswer
...
rooms/{roomCode}/teams/{teamId}         — name, emoji, score, memberNames[], audioChoice/audioChoiceRound (open read/write among signed-in users — private friends game, not defended against cheating)
...
No turn order, no "active team" — every team decides and answers independently every round (see milestone 8). `audioOnly` on a guess doc is set by the player when they choose "Answer now" during the audio phase, and drives that team's own double-points bonus at reveal — it's per-guess, not a room-wide flag, since different teams can take either path in the same round.
